...
function fish_prompt
  ...
  echo -sn $purple$__fish_user' '$blue$USER$white
  echo -sn ' in '$yellow$cwd

  if test $cwd != '~'; or test -n "$lucid_git_status_in_home_directory"
    set -l git_state (__lucid_git_status)
    if test $status -eq 0
      echo -sn $white' on '$git_state
    end
  end

  __lucid_vi_indicator

  set -l __prompt $greeno$__fish_prompt_char$greeno' ' 
  set -l __exit_code ''
  for status_code in $last_pipestatus
    if test $status_code -ne 0
     set __prompt $redo'x '
     set __exit_code ' '$redo$status_code
     break
    end 
  end

  echo -sne $__exit_code' '$normal$__time'\n'$__prompt
end
